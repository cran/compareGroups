...
\title{\Large \bf {\tt compareGroups} 3.0: Descriptives by groups \\ \vspace{0.5cm}}



\author{Isaac Subirana$^{3,1,4}$ , Joan Vila$^{1,3}$ and H\'ector Sanz$^{2}$}
%\VignetteIndexEntry{compareGroups}

\maketitle

\begin{scriptsize}
\noindent $^{1}$IMIM - Parc de Salut Mar, Barcelona \\
$^{2}$Barcelona Centre for International Health Research (CRESIB, Hospital Cl\'inic-Universitat de Barcelona), Barcelona, Spain \\
...
The \textbf{{\tt compareGroups}} package \cite{Subirana2014} allows users to create tables displaying results of univariate analyses, stratified or not by categorical variable groupings.\\

Tables can easily be exported to CSV, \LaTeX{}, HTML or PDF. \\

This package can be used from the \Rlogo~ prompt or from a user-friendly GUI.\\

Since version 3.0, a Web User Interface (WUI) has been implemented based on {\tt Shiny} package (\cite{Shiny}) which makes the functionality even more friendly. Also, this can be used remotely from \texttt{www.comparegroups.eu}. See the WUI vignette (`compareGroupsWUI\_vignette.pdf') for more details.\\
...
\item summarized
\end{itemize*}
\item {\tt export2csv}, {\tt export2html}, {\tt export2latex} and {\tt export2pdf} will export results to CSV, HTML, \LaTeX{} or PDF, respectively. 
\end{itemize*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Substracting results} \label{substr}
...
For example, it may be interesting to recover the p-values for each variable as a vector to further manipulate it in R, like adjusting for multiple comparison with {\tt p.adjust}. For example, lets take the data {\tt SNPassoc} that contains information of dozens of SNPs (genetic variants) from a sample of cases and controls. In this case we analize five of them:
...
\paragraph{header.labels} Change some key table header, such as the p.overall,.... Note that this is done when printing the table changing the argument in the `print' function and not in the `createTable' function. This argument is also present in other function that exports the table to pdf, plain text, etc.
...
Tables can be exported to CSV, HTML, \LaTeX{} or PDF:

\begin{itemize*}
\item export2csv(restab, file="table1.csv"), exports to CSV format
\item export2html(restab, file="table1.html"), exports to HTML format
\item export2latex(restab, file="table1.tex"), exports to \LaTeX{} format
\item export2pdf(restab, file="table1.pdf"), exports to PDF format
\end{itemize*}
...
%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\subsection{Missing values} \label{missing}
...
%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\subsection{Analysis of genetic data} \label{genetic}
...
Once the {\tt compareGroups} package is loaded, a Graphical User Interface (GUI) is displayed in response to typing {\tt cGroupsGUI(predimed)}. The GUI is meant to make it feasible for users who are unfamiliar with \Rlogo~ to construct bivariate tables.
Note that, since version 3.0, it is necessary to specifiy an existing data.frame as input. So, for example, you can load the PREDIMED data by typing {\tt data(predimed)} before calling `cGroupsGUI' function.
\\
