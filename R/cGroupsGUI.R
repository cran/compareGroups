...
cGroupsGUI <- function(X){
    ...
    exclude.var <- function(){
      ...
      report.options <- function(){
             ...
             results.value <- tclVar(get(".p.trend",envir=.compareGroupsEnv,mode="any", inherits = FALSE))
             results.value2 <- tclVar(get(".p.mult",envir=.compareGroupsEnv,mode="any", inherits = FALSE))
             results.value3 <- tclVar(get(".p.overall",envir=.compareGroupsEnv,mode="any", inherits = FALSE))
             ...
             results.n.value <- tclVar(get(".show.n",envir=.compareGroupsEnv,mode="any", inherits = FALSE))
             ...
             results.value.all <- tclVar(get(".show.all",envir=.compareGroupsEnv,mode="any", inherits = FALSE))
             ...
             results.value.desc <- tclVar(get(".show.desc",envir=.compareGroupsEnv,mode="any", inherits = FALSE))
             ...
             results.value.haz <- tclVar(get(".show.haz",envir=.compareGroupsEnv,mode="any", inherits = FALSE))
             ...
             type.cat.valuex <-  tclVar(get(".type.cat.value",envir=.compareGroupsEnv,mode="any", inherits = FALSE))
             ...
             tkgrid(tklabel(topreportframe,text="",fg="blue", font=fontype),results.all, sticky="nw")
                 accept.fun <- function(){
                  assign(".p.overall" ,tclvalue(results.value3) , envir = .compareGroupsEnv)
                  assign(".p.trend" ,tclvalue(results.value) , envir = .compareGroupsEnv)
                  assign(".p.mult" ,tclvalue(results.value2) , envir = .compareGroupsEnv)
                  assign(".show.all" ,tclvalue(results.value.all) , envir = .compareGroupsEnv)
                  assign(".show.desc" ,tclvalue(results.value.desc) , envir = .compareGroupsEnv)
                  assign(".show.haz" ,tclvalue(results.value.haz) , envir = .compareGroupsEnv)
                  assign(".show.n"  ,tclvalue(results.n.value) , envir = .compareGroupsEnv)
                  assign(".type.cat.value"  ,tclvalue(type.cat.valuex) , envir = .compareGroupsEnv)
                 tkdestroy(report)
            }
            cancel.fun <- function(){
                tkdestroy(report)
     ...
     export2html.aux <- function(){
        ...
        ans <- try(eval(parse(text= paste("compareGroups(X = res$X, y = res$y, Xext=res$Xext, selec =", res$selec,", method = res$method, alpha = res$alpha,min.dis = 5, max.ylev = 5, ref.y = res$ref.y, ref= res$ref)"))))
          ...
          if(inherits(ans,"try-error")) return(tkmessageBox(message = "Error occurred in 'export2html' function", icon = "info", type="ok"))
          if(!inherits(ans,"try-error")) return(tkmessageBox(message = "Correct 'export2html'", icon = "info", type="ok"))
     }     
     topMenu <- tkmenu(tt)
     ...
     tkadd(fileMenu2,"command",label="HTML",command=export2html.aux)
