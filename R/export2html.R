export2html<-function(x, file, which.table="descr", nmax = TRUE, ...){
    ...
    if (ww %in% c(1, 3)) {
        ...
        table1[, 1] <- sub("^    ", "&nbsp;&nbsp;&nbsp;&nbsp;", 
            table1[, 1])
        if (nrow(table1) > 1 && length(grep("^N=", trim(table1[2, 
            2])))) {
            wn <- grep("^N=", trim(table1[2, ]))
            nn <- paste(trim(table1[1, wn]), "<br/>", trim(table1[2, 
                wn]))
            table1[1, wn] <- nn
            table1 <- table1[-2, ]
        }
        ...
        print(xtable(table1, align = align), type = "html", file = paste(file, 
            ".html", sep = ""), include.rownames = FALSE, sanitize.text.function = function(x) x)
    }
    if (ww %in% c(2, 3)) {
        ...
        print(xtable(table2, align = align), type = "html", file = paste(file, 
            "_appendix.html", sep = ""), include.rownames = FALSE, 
